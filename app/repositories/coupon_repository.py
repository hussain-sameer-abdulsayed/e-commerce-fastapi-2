...
class CouponRepository:
   ...
   async def get_coupons_by_active_status(self, is_active: bool) -> List[Coupon]:
      now = datetime.now()
   ...
   async def set_coupon_active_status(self, id: UUID, coupon_set_status: coupon.CouponSetStatus) -> bool:
      coupon = await self.get_by_id(id)
      if not coupon or coupon.is_active == coupon_set_status.is_active:
         return False
      
      coupon.is_active = coupon_set_status.is_active
      self.db.add(coupon)
      await self.db.commit()
   ...
   async def get_coupon_usages_by_user_id(self, user_id: UUID) -> List[CouponUsage]:
      statement = (
         select(CouponUsage)
